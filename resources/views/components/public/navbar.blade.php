@php
    ...
    $links = [
        ...
        ['label' => 'Tools', 'url' => route('tools.index'), 'active' => request()->routeIs('tools.*')],
...
<header id="site-header" class="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--bg)] backdrop-blur-xl transition-shadow duration-200">
    <div class="safe-container flex min-h-[68px] items-center justify-between gap-3 py-3">
        ...
        <nav class="hidden items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--surface)] p-1 text-sm font-bold shadow-sm xl:flex" aria-label="Primary navigation">
    ...
    <div id="mobile-menu" data-mobile-menu class="mobile-menu-panel hidden border-t border-[var(--border)] bg-[var(--bg)] shadow-2xl xl:hidden">
        <nav class="safe-container grid gap-2 py-4 text-sm font-bold" aria-label="Mobile navigation">
            ...
            @endforeach
            <a href="{{ $brand['portfolio_url'] }}" data-mobile-menu-link class="premium-button mt-2 bg-[var(--accent)] text-white" rel="noopener noreferrer">Work With Me</a>
        </nav>
    </div>
</header>
