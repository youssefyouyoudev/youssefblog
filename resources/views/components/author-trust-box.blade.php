...
<section {{ $attributes->merge(['class' => 'rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-soft']) }}>
    <div class="flex gap-4 {{ $compact ? 'items-center' : 'items-start' }}">
        ...
        @else
            <div class="{{ $compact ? 'h-14 w-14' : 'h-20 w-20' }} flex shrink-0 items-center justify-center rounded-full bg-[var(--accent)] text-xl font-black text-white">YY</div>
        @endif
        <div>
