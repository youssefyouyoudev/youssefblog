...
<article {{ $attributes->merge(['class' => 'group rounded-2xl border border-black/10 bg-white p-6 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-emerald-500 hover:shadow-glow']) }}>
    <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 text-white transition group-hover:-translate-y-0.5 group-hover:bg-blue-700">
        @switch($icon)
            @case('chart')
    ...
    <a href="{{ $url }}" class="mt-5 block text-xl font-black text-ink transition hover:text-emerald-700">{{ $name }}</a>
