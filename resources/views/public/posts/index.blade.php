        ...
        <div class="safe-container py-10 lg:py-12">
            ...
            <div class="mt-8 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
                <div>
                    ...
                    <h1 class="mt-3 text-[clamp(2rem,9vw,3rem)] font-black tracking-tight text-[var(--text)] sm:text-5xl">{{ $heading ?? 'Latest Articles' }}</h1>
                ...
                <form method="GET" action="{{ route('posts.index') }}" class="rounded-2xl border border-[var(--border)] bg-[var(--bg)] p-4 shadow-soft">
                    ...
                    <div class="mt-3 flex flex-col gap-2 min-[430px]:flex-row">
                        <input id="q" name="q" value="{{ $search }}" type="search" placeholder="Laravel SEO, AI tools..." class="min-h-12 min-w-0 flex-1 border border-[var(--border)] bg-[var(--surface)] px-4 text-base text-[var(--text)] focus:border-[var(--accent)]">
                        <button class="premium-button bg-[var(--accent)] text-white" type="submit">Search</button>
                    </div>
                </form>
            </div>
            <div class="scrollbar-soft mt-8 flex gap-2 overflow-x-auto pb-2">
